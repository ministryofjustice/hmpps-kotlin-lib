...
- org.springframework.boot [3.5.7 -> 3.5.8]
...
- io.opentelemetry.instrumentation:opentelemetry-instrumentation-annotations [2.21.0 -> 2.19.0]
- org.springdoc:springdoc-openapi-starter-common [2.8.13 -> 2.8.14]

# 1.8.1

## Build plugin version updates

- Kotlin JVM/Spring [2.2.20 -> 2.2.21]
- com.github.ben-manes.versions [0.52.0 -> 0.53.0]
...
# 1.8.0
Added new Subject Access Request endpoint `/subject-access-request/template` (disabled by default). When enabled the 
endpoint will return your service's subject access request report mustache template file. These templates will be 
retrieved by the Subject Access Request service at runtime and used to present the raw data in human-readable format 
when generating report PDFs.

See [Subject Access Request Template configuration](../readme-contents/SubjectAccessRequestEndpoint.md/#subject-access-request-template-endpoint-configuration)
for details on enabling/configuring the subject access request template endpoint.

# 1.7.0

Updates the `HmppsSubjectAccessRequestContent` class, used by the controllers `HmppsSubjectAccessRequestController`
and `HmppsSubjectAccessRequestReactiveController` which define the standard SAR API needed for services, to allow
for attachment data to also be optionally returned if it exists. 

## Dependency version updates:
- org.springframework.boot:spring-boot-dependencies [3.5.5 -> 3.5.6]
- org.jetbrains.kotlin:kotlin-stdlib [2.2.10 -> 2.2.20]

# 1.6.0

Modifies the default `OAuth2AuthorizedClientManager`/`ReactiveOAuth2AuthorizedClientManager` beans to use
`GlobalPrincipalOAuth2AuthorizedClientService`/`ReactiveGlobalPrincipalOAuth2AuthorizedClientService`
respectively as the `OAuth2AuthorizedClientService`/`OAuth2AuthorizedClientService`. These authorized
client services are configured to cache client credentials tokens under a global principal name instead
of the current authenticated principal. This is to reduce the number of unnecessary client credentials
token request calls.

The non-reactive client provider has also been changed so that it no longer uses the deprecated
`DefaultClientCredentialsTokenResponseClient` that uses `RestTemplate` under the covers
and instead uses the new spring `RestClientClientCredentialsTokenResponseClient` that uses `RestClient` instead.

Two new client managers have also been added, which will pass through the current `username`.
The `usernameAwareTokenRequestOAuth2AuthorizedClientManager` needs to be defined in a new `WebClient`
at `@RequestScope` so that it can access the current authorisation each time.  The reactive version
is `reactiveUsernameAwareTokenRequestOAuth2AuthorizedClientManager`.

## Dependency version updates:
- org.springframework.boot:spring-boot-dependencies [3.5.4 -> 3.5.5]
- org.springdoc:springdoc-openapi-starter-common [2.8.9 -> 2.8.13]
- io.opentelemetry.instrumentation:opentelemetry-instrumentation-annotations [2.18.0 -> 2.18.1]
...
Add a default 30 second timeout when retrieving client credentials, and make it overridable.
...
## Dependency version updates:
- io.opentelemetry.instrumentation:opentelemetry-instrumentation-annotations [2.16.0 -> 2.18.0]
