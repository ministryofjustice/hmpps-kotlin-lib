# 1.8.0
Added new Subject Access Request endpoint `/subject-access-request/template`. New endpoint will be automatically created
if service code implements one of the subject access request service interfaces:
- `HmppsSubjectAccessRequestReactiveService`
- `HmppsSubjectAccessRequestService`

Endpoint returns your service's subject access request report mustache template file. These templates will be retrieved 
by the Subject Access Request service at runtime and used to present the raw data in human-readable format when 
generating report PDFs.

See [Subject Access Request Template configuration](../readme-contents/SubjectAccessRequestEndpoint.md/#subject-access-request-template-configuration)
for details on configuring the subject access request template endpoint. The template endpoint will return `status 500` 
