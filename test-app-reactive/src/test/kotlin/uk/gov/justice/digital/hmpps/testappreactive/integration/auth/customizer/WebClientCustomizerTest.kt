...
import uk.gov.justice.digital.hmpps.testappreactive.integration.wiremock.HmppsAuthApiExtension.Companion.hmppsAuth
...
class WebClientCustomizerTest(
  ...
  class CustomizerConfiguration {
    ...
    ): ReactiveOAuth2AuthorizedClientProvider = builder.reactiveOAuth2AuthorizedClientProvider(Duration.ofMillis(50))
  }

  @Test
  fun `should return OK when exceeding the client credentials client timeout`() = runTest {
  ...
  @Test
  fun `should throw when exceeding the client credentials client timeout`() = runTest {
