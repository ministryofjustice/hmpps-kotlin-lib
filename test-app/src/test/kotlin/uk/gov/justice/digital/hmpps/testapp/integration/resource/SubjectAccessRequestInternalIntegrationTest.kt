 ...
 *
 * Other scenarios such as 209 or 400 scenarios should be left to this class to test as they are subject to change.
 */
class SubjectAccessRequestInternalIntegrationTest : IntegrationTestBase() {
  ...
  inner class SAREndpointWithNoProperty {
    ...
    @Test
    fun `should not return data if no additional access property set`() {
      webTestClient.get().uri("/subject-access-request?prn=A12345")
    ...
    @Test
    fun `should not return expected template if no additional access property set`() {
      webTestClient.get().uri("/subject-access-request/template")
        .headers(setAuthorisation(roles = listOf("ROLE_TEST_DATA_ACCESS")))
        .exchange()
        .expectStatus().isForbidden
  ...
  inner class SAREndpointWithProperty {
    ...
    @Test
    fun `should still return data if prisoner exists when SAR role set`() {
      webTestClient.get().uri("/subject-access-request?prn=A12345")
        .headers(setAuthorisation(roles = listOf("ROLE_SAR_DATA_ACCESS")))
        .exchange()
        .expectStatus().isOk
    ...
    @Test
    fun `should still return expected template when SAR role set`() {
      webTestClient.get().uri("/subject-access-request?prn=B12345C")
        .headers(setAuthorisation(roles = listOf("ROLE_SAR_DATA_ACCESS")))
        .exchange()
        .expectStatus().isNoContent
